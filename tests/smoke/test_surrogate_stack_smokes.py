...
from src.prediction.calibration import build_prediction_context
...
def test_primary_failure_short_circuits_and_emits_structured_failure():
    class _Fail(_Primary):
        def compose_prediction(self, *args, **kwargs):
            raise _NoFitError("does not fit")
    ...
    peers = _Peers()
    events = []
    composer = SurrogateComposer(_Fail(), peer_client=peers, event_sink=events.append)
    try:
    ...
    assert [event["event"] for event in events] == [
        "surrogate.prediction.started",
        "surrogate.prediction.failed",
    ]
    assert events[1]["failure"]["stage"] == "primary"
    assert events[1]["failure"]["error_type"] == "_NoFitError"
    assert events[1]["inputs"]["raw"]["job_config"] == CONFIG


def test_completed_event_and_learned_fusion_skip_residual_throughput_calibration():
    evidence = _Evidence()
    events = []
    peers = _Peers()
    composer = SurrogateComposer(
    ...
    assert events[0]["event"] == "surrogate.prediction.started"
    assert events[0]["inputs"]["raw"]["job_config"] == CONFIG
    assert events[0]["inputs"]["normalized"]["job_features"]
    assert events[1]["event"] == "surrogate.prediction.completed"
    assert events[1]["trace"] == cold_trace
