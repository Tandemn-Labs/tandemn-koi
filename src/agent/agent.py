...
class KoiAgentHarness:
    ...
    def build_root_prompt(
        ...
        return (
            ...
            "deterministically - it takes specialist ladders as HINTS, GENERATES a "
            "frame on every other viable GPU type (so a job whose specialist only "
            "proposed L40S still gets an H100 candidate), sizes each with "
            ...
            "A job in scored['exhausted'] has NO runnable frame at all; a job the "
            "solver simply leaves out lost the serve-vs-defer tradeoff. Record the "
