...
class SpecialistRunner:
    ...
    def _default_prompt(job_id: str, slice_: dict[str, Any], brief: dict[str, Any]) -> str:
        return (
            ...
            "CHOOSING THE GPU TYPE. Choose by which type meets the throughput need "
            "MOST EFFICIENTLY, not by whether the model just fits. Use what you "
            "know about each GPU's memory, bandwidth, and compute for THIS model's "
            "size: a large or throughput-heavy model usually needs FAR FEWER fast "
            "GPUs (e.g. H100) than slow ones (e.g. L40S) to hit the same target, "
            "which makes the faster GPU CHEAPER overall and higher quality. For "
            "each viable GPU type, estimate how many GPUs it takes to hit the "
            "target, and pick on that basis. Never avoid a GPU type to 'save it' - "
            "the cluster planner owns scarcity and will reassign if two jobs "
            "contend, so propose the type you genuinely believe serves this job "
            "best, even when that is the higher-end GPU.\n\n"
            "Task:\n"
            ...
            "to span several instances). Match the instance to your parallelism: for "
            "tp*pp = N pick an instance whose gpus_per_instance >= N (e.g. an 8-GPU "
            "p5.48xlarge for tp=8), NOT a 1-GPU instance with gpu_count=N. The UNIT "
    ...
    def _validate(
        ...
        if action_name in (ActionType.PLACE.value, ActionType.SWAP.value) and isinstance(
            ...
            # the retry self-correct (e.g. p5.4xlarge has 1 GPU/instance -> use
            # p5.48xlarge for a multi-GPU frame, or raise num_nodes_per_chain).
            try:
...
class KoiAgentHarness:
    ...
    def build_root_prompt(
        ...
        return (
            ...
            "optimize_config, jointly_select_placements, "
            ...
            "one per GPU TYPE it could use (e.g. an L40S frame AND an H100 frame), "
            "so a scarce type goes to whichever job gains most - then size and "
            ...
            "for: if a job wants H100, GIVE it an H100 candidate - do not box it "
            "onto whatever fits a slice. If you are about to defer most jobs, "
            ...
            "After specialists, derive waiting IDs ONLY from each job dict's job_id, "
            "never from list indexes: "
            "pending_jobs = [j for j in get_cluster_state()['pending_jobs'] "
            "if isinstance(j, dict) and j.get('job_id')]; "
            "jobs_by_id = {str(j['job_id']): j for j in pending_jobs}; "
            "waiting_job_ids = list(jobs_by_id); "
            "assert all(jid in specialist_results for jid in waiting_job_ids), waiting_job_ids. "
            ...
            "run_job_specialists. Each job's budget is now a PERMISSIVE upper "
            "bound - specialists see the FULL free pool (budgets OVERLAP, they "
            "are not exclusive slices), so each proposes the best GPU for ITS "
            "job; they do NOT resolve competition for shared GPUs - YOU do, "
            "jointly, via jointly_select_placements. Their "
            ...
            "is reporting a LOCAL verdict for its own frame - it is not a "
            "cluster decision. If you can construct a feasible placement the "
            "specialist did not try (especially on a GPU type it skipped), do it "
            "and ADD it to the candidate set you hand the joint solver. Inherit a "
            "defer only when the joint solver still places nothing. Treat fitness, "
            "marginal_value_of_more, and any "
            ...
            "selection across ALL waiting jobs against the one shared pool. It "
            "ARBITRATES the frames you pass - it does NOT invent them, so you MUST "
            "include a candidate for each GPU type worth considering (a big model "
            "needs an H100 frame IN the list, not just L40S). candidates is a "
            ...
            "Worked JOINT pipeline (decide all jobs together):\n"
            "    cands = []\n"
            "    for jid in waiting_job_ids:\n"
            "        for env in gpu_type_envs_this_job_may_use:  # an L40S env AND an H100 env\n"
            "            sized = size_ladder(ranks_for(jid, env), features_for(jid))\n"
            "            if not sized['ranks']: continue\n"
            "            act = {'job_id': jid, 'type': 'place', 'user_id': uid,\n"
            "                   'ladder': sized['ranks'], 'target_tps': sized['target_tps'],\n"
            "                   'mechanism_id': mid, 'budget_ref': slice_id}\n"
            "            one = {'tick_rationale': '...', 'actions': [act]}\n"
            "            if not check_feasibility(one)['feasible']: continue\n"
            "            act['sigma'] = compute_sigma(one)['per_job'][jid]['sigma']\n"
            "            cands.append(act)\n"
            "    result = jointly_select_placements(cands)\n"
            "    plan = {'tick_rationale': '...', 'actions': result['chosen']}\n"
            "    # plan is joint-optimal; confirm the whole thing before committing\n"
