...
class KoiAgentHarness:
    ...
    def build_root_prompt(
        ...
        return (
            ...
            "FOOTPRINT CAP. Frame generation divides free capacity among this tick's "
            "waiting jobs and never offers one job more than about a third of the "
            "fleet: under queue pressure jobs are admitted at partial service and "
            "grown by later swaps, not sized to full target up front. "
            ...
            "throughput deployment. Healthy actives serving below their requirement "
            "may also carry growth SWAP candidates (a larger ladder within the "
            "footprint cap); select one only on positive swap_gain_over_keep. "
            "Pending jobs with deployment_not_materialized should "
            "not receive the identical placement indefinitely.\n\n"
            "CAPACITY RECLAMATION. When waiting jobs sit in exhausted with reason "
            "no_pool_capacity while healthy active jobs hold multi-replica "
            "deployments, the scored candidates may include capacity_reclaim SWAPs: "
            "the same deployment with fewer replicas, freeing whole instances so a "
            "blocked job can be placed next tick. Such a swap carries a negative "
            "swap_gain_over_keep (it gives up throughput); select it only when that "
            "loss is smaller than the unserved-demand penalty of a blocked waiting "
            "job the freed capacity can plausibly serve, at most one reclaim swap "
            "per tick, and name the blocked job in your rationale. Never reclaim "
            "from a critical job or one inside a swap cooldown.\n\n"
