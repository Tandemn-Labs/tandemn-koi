...
def calibrate_prediction(
    ...
    *,
    ...
    ranked = []
    for row in _rows_before(evidence_store, as_of_timestamp_utc):
...
def _rows_before(evidence_store, as_of: float) -> list:
    if not hasattr(evidence_store, "get_all_rows"):
        return []
    return [
        row
        for row in evidence_store.get_all_rows(limit=None)
